...
- [X] Dogfood: register agent-workbench as its own project in the workbench once pre-dogfood fixes are done; use `awb` to manage all remaining implementation tasks instead of editing TODO.md directly. Completed 2026-05-23 by claude-sonnet-4-6; project seeded via seed_dev.py; 7 stale pending tasks synced to completed via `awb task complete`; dogfood task completed in workbench. **Task management now via `awb` — TODO.md is read-only reference from this point.**
